...
contract FlashLoan is IERC3156FlashLender {
   ...
   function flashFee(
      ...
      //@audit -> Is not validating the input token var mathces the address of this contract!
       return fee;  // Is not calculating a fee ? Just returning the plane fee variable!
   }
   ...
   function flashLoan(
       ...
       if(newBal < oldBal + fee) {
           ...
           // @audit => pending tokens to be paid are finally paid!
       ...
       if (IERC20(token).allowance(address(this), address(receiver)) > oldAllowance) {
          // @audit => Use of approve() function instead of increaseAllowance and decreaseAllowance to prevent front running!
